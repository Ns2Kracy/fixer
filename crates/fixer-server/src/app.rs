...
pub fn app() -> Router {
    observability::observe(api_app(crate::api::v1::router()))
}

/// Builds the complete router with persistent job APIs enabled.
pub fn job_app(runtime: JobRuntime) -> Router {
    observability::observe(api_app(crate::api::v1::job_router(runtime)))
}
...
pub fn workspace_app(state: WorkspaceState) -> Router {
    observability::observe(api_app(crate::api::v1::workspace_router(state)))
}
