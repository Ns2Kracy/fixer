...
const terminalStates = new Set<JobState>(['failed', 'cancelled', 'interrupted'])

export function ProgressTimeline(props: { state: JobState; progress: ProgressSummary | undefined }) {
  const currentIndex = () => {
    const index = stages.findIndex(([state]) => state === props.state)
    if (index >= 0) return index
    const progressIndex = stages.findIndex(([state]) => state === props.progress?.stage)
