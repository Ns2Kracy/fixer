...
describe("jobs workflow", () => {
  ...
  it.each([
    ...
    "shows valid actions for %s jobs",
    async (state, canRetry, canCancel) => {
      ...
      expect(
        Boolean(
          screen.queryByRole("button", { name: "Cancel before writing" }),
        ),
      ).toBe(canCancel);
      view.unmount();
    },
          ...
          ? [
              {
                ...
                id: 8,
                state: "interrupted",
                input: { ...job.input, input_path: "/media/B.mkv" },
              },
            ]
          : [
              job,
  ...
  it("compares candidates, preserves partial warnings, and acknowledges sourced conflicts", async () => {
      ...
      warnings: [
        {
          ...
          message: "TMDB timed out; local metadata remains usable.",
        },
      ],
    ...
    expect(
      await screen.findByRole("heading", { name: "Review metadata" }),
    ).toBeVisible();
    expect(
    ...
    await user.click(
      screen.getByRole("radio", {
        name: "Select Candidate One from fixture.local",
      }),
    );
    await user.click(
      screen.getByRole("radio", {
    ...
    expect(
      screen.getByText("Additional source context was omitted by the server."),
    ).toBeVisible();
    await user.click(
      screen.getByRole("checkbox", { name: "Acknowledge conflict summaries" }),
    );
    await user.click(
      screen.getByRole("button", { name: "Accept candidate and build plan" }),
    );

    await waitFor(() => {
      const request = fetchMock.mock.calls.find(
        ([input, init]) =>
          String(input).endsWith("/jobs/7/review") && init?.method === "POST",
      );
  ...
  it("previews filesystem operations and requires explicit approval before execution", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.endsWith("/jobs/7/execute") && init?.method === "POST") {
          return json({
            schema_version: 1,
            job: { ...planningJob, state: "completed" },
          });
        }
        if (url.endsWith("/jobs/7/plan")) {
          return json({
    ...
    expect(
      await screen.findByRole("heading", { name: "Output plan" }),
    ).toBeVisible();
    ...
    await waitFor(() => {
      const request = fetchMock.mock.calls.find(
        ([input, init]) =>
          String(input).endsWith("/jobs/7/execute") && init?.method === "POST",
      );
      expect(request?.[1]?.headers).toEqual(
        expect.objectContaining({ "idempotency-key": "job-7-execution-uuid" }),
      );
      expect(JSON.parse(String(request?.[1]?.body))).toEqual({
        approved: true,
      });
